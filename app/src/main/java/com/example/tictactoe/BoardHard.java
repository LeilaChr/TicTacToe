...
class BoardHard extends View {
    ...
    public BoardHard(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        game = new Game();

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.BoardSingle, 0, 0);

        try {
            gameBoardColor = a.getInteger(R.styleable.BoardSingle_color, 0);
            XColor = a.getInteger(R.styleable.BoardSingle_xcolor, 0);
            OColor = a.getInteger(R.styleable.BoardSingle_ocolor, 0);
            diagonalColor = a.getInteger(R.styleable.BoardSingle_winlinecolor, 0);
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (!winningLine) {
                ...
                handler.postDelayed(new Runnable() {
                    public void run() {
                        HashMap d = game.doComputer();
                        Object c = d.keySet().toArray()[new Random().nextInt(d.keySet().toArray().length)];
                        int move = game.findBestMove(game.getGameBoard());
                        if (game.updateGameBoard((move/3) +1, (move%3)+1)) {
                            invalidate();
                            if (game.winnerCheck()) {
                                winningLine = true;
                                invalidate();
